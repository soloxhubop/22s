...
local FileName = "MeloskaSave_v1.json" 
local DefaultConfig = {
    Positions = {
...
local function kickPlayer()
    LocalPlayer:Kick("\ndiscord.gg/fkdNuQq4e - xi loves you <3")
end
...
local function ShowNotification(title, text)
    local existing = PlayerGui:FindFirstChild("MelNotif")
    if existing then existing:Destroy() end

    local sg = Instance.new("ScreenGui", PlayerGui)
    sg.Name = "MelNotif"; sg.ResetOnSpawn = false
...
task.spawn(function()
    ...
    local function ShowPriorityAlert(brainrotName, genText, mutation, ownerUsername)
        ...
        local existing = PlayerGui:FindFirstChild("MelAlert")
        if existing then existing:Destroy() end
        
        local alertGui = Instance.new("ScreenGui")
        alertGui.Name = "MelAlert"
...
if IS_MOBILE then
    ...
    createMobBtn("UI", Color3.fromRGB(255, 255, 255), 5, function()
        ...
        local adUI = PlayerGui:FindFirstChild("MelAdminPanel")
...
bReset.MouseButton1Click:Connect(function()
    ...
    if PlayerGui:FindFirstChild("MelAdminPanel") and PlayerGui.XiAdminPanel:FindFirstChild("Frame") then
...
-- ============================================================
-- DESYNC PANEL (stile Invis Panel, auto-desync durante steal)
-- ============================================================
do
    _G.desyncEnabled = false
    _G.autoDesyncDuringSteal = false

    local desyncGui = Instance.new("ScreenGui")
    desyncGui.Name = "MelDesyncPanel"
    desyncGui.ResetOnSpawn = false
    desyncGui.Parent = PlayerGui

    local dFrame = Instance.new("Frame", desyncGui)
    dFrame.Size = UDim2.new(0, 200, 0, 160)
    dFrame.Position = UDim2.new(Config.Positions.InvisPanel.X + 0.15, 0, Config.Positions.InvisPanel.Y, 0)
    ...
    dContainer.Size = UDim2.new(1, -20, 1, -40)
    dContainer.Position = UDim2.new(0, 10, 0, 35)
    ...
    local function CreateDRow(height)
        local r = Instance.new("Frame", dContainer)
        r.Size = UDim2.new(1, 0, 0, height or 30)
        r.BackgroundTransparency = 1
        return r
    end
    ...
    -- Row 1: Toggle Desync
    local row1 = CreateDRow(30)
    local lbl1 = Instance.new("TextLabel", row1)
    lbl1.Size = UDim2.new(0.6, 0, 1, 0)
    lbl1.BackgroundTransparency = 1
    lbl1.Text = "Toggle Desync"
    lbl1.TextColor3 = Theme.TextPrimary
    lbl1.Font = Enum.Font.GothamBold
    lbl1.TextSize = 12
    lbl1.TextXAlignment = Enum.TextXAlignment.Left

    local btnDesync = Instance.new("TextButton", row1)
    btnDesync.Size = UDim2.new(0, 40, 0, 24)
    btnDesync.Position = UDim2.new(1, -40, 0.5, -12)
    btnDesync.BackgroundColor3 = Theme.SurfaceHighlight
    btnDesync.Text = "OFF"
    btnDesync.Font = Enum.Font.GothamBold
    btnDesync.TextSize = 11
    btnDesync.TextColor3 = Theme.TextPrimary
    Instance.new("UICorner", btnDesync).CornerRadius = UDim.new(0, 6)

    local function setDesyncVisual(on)
        btnDesync.Text = on and "ON" or "OFF"
        btnDesync.BackgroundColor3 = on and Theme.Success or Theme.SurfaceHighlight
    end

    btnDesync.MouseButton1Click:Connect(function()
        _G.desyncEnabled = not _G.desyncEnabled
        pcall(function() raknet.desync(_G.desyncEnabled) end)
        setDesyncVisual(_G.desyncEnabled)
    end)

    -- Row 2: Auto Desync During Steal
    local row2 = CreateDRow(30)
    local lbl2 = Instance.new("TextLabel", row2)
    lbl2.Size = UDim2.new(0.6, 0, 1, 0)
    lbl2.BackgroundTransparency = 1
    lbl2.Text = "Auto During Steal"
    lbl2.TextColor3 = Theme.TextPrimary
    lbl2.Font = Enum.Font.GothamBold
    lbl2.TextSize = 12
    lbl2.TextXAlignment = Enum.TextXAlignment.Left

    local btnAutoDesync = Instance.new("TextButton", row2)
    btnAutoDesync.Size = UDim2.new(0, 40, 0, 24)
    btnAutoDesync.Position = UDim2.new(1, -40, 0.5, -12)
    btnAutoDesync.BackgroundColor3 = Theme.SurfaceHighlight
    btnAutoDesync.Text = "OFF"
    btnAutoDesync.Font = Enum.Font.GothamBold
    btnAutoDesync.TextSize = 11
    btnAutoDesync.TextColor3 = Theme.TextPrimary
    Instance.new("UICorner", btnAutoDesync).CornerRadius = UDim.new(0, 6)

    btnAutoDesync.MouseButton1Click:Connect(function()
        _G.autoDesyncDuringSteal = not _G.autoDesyncDuringSteal
        btnAutoDesync.Text = _G.autoDesyncDuringSteal and "ON" or "OFF"
        btnAutoDesync.BackgroundColor3 = _G.autoDesyncDuringSteal and Theme.Success or Theme.SurfaceHighlight
    end)

    -- Row 3: Auto Fix Lagback
    local row3 = CreateDRow(30)
    local lbl3 = Instance.new("TextLabel", row3)
    lbl3.Size = UDim2.new(0.6, 0, 1, 0)
    lbl3.BackgroundTransparency = 1
    lbl3.Text = "Auto Fix Lagback"
    lbl3.TextColor3 = Theme.TextPrimary
    lbl3.Font = Enum.Font.GothamBold
    lbl3.TextSize = 12
    lbl3.TextXAlignment = Enum.TextXAlignment.Left

    local btnLagback = Instance.new("TextButton", row3)
    btnLagback.Size = UDim2.new(0, 40, 0, 24)
    btnLagback.Position = UDim2.new(1, -40, 0.5, -12)
    btnLagback.BackgroundColor3 = Theme.SurfaceHighlight
    btnLagback.Text = "OFF"
    btnLagback.Font = Enum.Font.GothamBold
    btnLagback.TextSize = 11
    btnLagback.TextColor3 = Theme.TextPrimary
    Instance.new("UICorner", btnLagback).CornerRadius = UDim.new(0, 6)

    local autoFixLagback = false
    btnLagback.MouseButton1Click:Connect(function()
        autoFixLagback = not autoFixLagback
        btnLagback.Text = autoFixLagback and "ON" or "OFF"
        btnLagback.BackgroundColor3 = autoFixLagback and Theme.Success or Theme.SurfaceHighlight
    end)

    -- Auto desync durante steal (collega al segnale Stealing)
    local desyncAutoEnabled = false
    LocalPlayer:GetAttributeChangedSignal("Stealing"):Connect(function()
        local isStealing = LocalPlayer:GetAttribute("Stealing")
        if _G.autoDesyncDuringSteal then
            if isStealing and not desyncAutoEnabled then
                desyncAutoEnabled = true
                _G.desyncEnabled = true
                pcall(function() raknet.desync(true) end)
                setDesyncVisual(true)
            elseif not isStealing and desyncAutoEnabled then
                desyncAutoEnabled = false
                -- fix lagback prima di disattivare
                if autoFixLagback then
                    local char = LocalPlayer.Character
                    local hrp = char and char:FindFirstChild("HumanoidRootPart")
                    if hrp then
                        local savedVel = hrp.AssemblyLinearVelocity
                        hrp.AssemblyLinearVelocity = Vector3.new(0, savedVel.Y, 0)
                        task.wait(0.05)
                    end
                end
                _G.desyncEnabled = false
                pcall(function() raknet.desync(false) end)
                setDesyncVisual(false)
            end
        end
    end)
end
